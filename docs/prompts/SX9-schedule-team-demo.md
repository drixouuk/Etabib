...
const [simulatedRole, setSimulatedRoleState] = useState<"doctor" | "secretary">(
  () => {
    ...
    return (
      (sessionStorage.getItem("demo-role") as "doctor" | "secretary") ||
      "doctor"
    );
  },
);
...
const effectiveTier =
  simulatedTier && simulatedTier !== "cabinet" ? simulatedTier : tier;
const effectiveRoles =
  simulatedRole === "secretary" ? ["secretary"] : user.roles;
```
...
```tsx
{
  isDemo && (
    <DemoSimulator
      ...
      onRoleToggle={() =>
        setSimulatedRole((s) => (s === "doctor" ? "secretary" : "doctor"))
      }
