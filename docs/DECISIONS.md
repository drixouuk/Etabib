...
- **Fix structurel** : **isolation par credentials dans la clé de cache** — `fetch` inclut le header `Authorization` dans les options, donc dans la clé du Data Cache Next (aucune entrée partagée entre tokens, prouvé par le test bi-compte) ; tags `auth:<hashFnv1a-du-token>` (jamais le token brut) pour le profil utilisateur, purgés au logout ; `authenticate()` enveloppé dans `cache()` React (dédup par requête : layout + page + route API = 1 seul users/me) ; lectures `fetchCMS` en TTL 30 s par défaut avec tags `col:<collection>` dérivés du chemin ; **toute écriture réussie revalide le tag de sa collection** (proxy CMS, bookings, annulation, export) — l'auteur voit sa donnée instantanément, les autres postes restent bornés par le TTL. **Isolation validée en double** : test serveur curl bi-compte (drdemo t4 vs drguinane t1 — B ne voit aucun résidu de A, la clé inclut bien le header) **et** purge du router cache client au login (`router.refresh()` — le seul vecteur de fuite restant était client : après un changement de compte dans le même onglet, les segments RSC mis en cache par le compte précédent resservaient jusqu'au prochain rafraîchissement) ; logout redirigé vers `/{locale}/login` (le `/login` nu enchaînait un POST→307 vers une page vide).
